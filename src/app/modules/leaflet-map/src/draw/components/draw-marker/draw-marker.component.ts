...
import { DrawMarkerHandler } from '../../handlers';
import { Map as LeafletMap } from 'leaflet';
import { DrawMarkerEvents } from '../../enums';

@Component({
  selector: 'app-draw-marker',
  templateUrl: './draw-marker.component.html',
  styleUrls: ['./draw-marker.component.scss'],
})
export class DrawMarkerComponent implements AfterViewInit {
  @Input() handler: DrawMarkerHandler | undefined;
  ...
  ngAfterViewInit(): void {
    ...
    this.map?.on(DrawMarkerEvents.DRAW_MARKER_START, () => this.drawStart());
    this.map?.on(DrawMarkerEvents.DRAW_MARKER_END, () => this.drawEnd());
    this.map?.on(DrawMarkerEvents.DRAW_MARKER_EDITED, () => this.polylineEdited());
  }
